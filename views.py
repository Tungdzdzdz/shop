        ...
        if request.method == "POST":
            ...
            try:
                ...
                try:
                    ...
                    message1 = "Price must be integer!"
                    error = True
                try:
                    sale = int(sale)
                except:
                    message2 = "Sale must be integer!"
                    error = True
                if error:
                    raise ValueError
            except:
                categories = Category.objects.all()
                context = {'categories': categories, 'message1': message1, 'message2': message2, 'name': name, 'price' : price, 'sale' : sale, 'description' : description, 'sizes':sizes, 'category1' : category}
            ...
            product = Product(name=name, image=relative_path, price=price, description=description, category=category,
                              sellerId=user, sizes=sizes, sale=sale)
            product.save()
            return redirect('product')
        return render(request, 'addProduct.html', {'categories': Category.objects.all()})
    return redirect('login')
    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            try:
                ...
                categories = Category.objects.all()
                context = {'product': product, 'categories': categories, 'message1': message1, 'message2': message2}
...
def cart(request):
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            if len(orderitems) == 0:
                return render(request, 'cart.html', {'message':'Please choose at least one product to buy!', 'profile': user})
            for orderitem in orderitems:
                if orderitem.size == None:
                    context = {'profile': user, 'order': order, 'orderitems': orderitems,'message': 'Please choose size for your items'}
            ...
            for orderitem in orderitems:
                ...
                orderitem.save()
            context = {'profile': user, 'order': order, 'orderitems': orderitems}
...
def customerOrder(request):
    if request.user.is_authenticated:
        ...
        context = {'orders': orders, 'profile': user}
...
def detailShop(request):
    if request.user.is_authenticated:
        ...
        productId = request.GET['id']
        product = Product.objects.get(id=productId)
        context = {'product': product, 'categories': categories, 'order': order, 'account': account,
...
def send_email(request):
    confirmation_url = 'Register to seller successfully!\nThank you for register. Click the link below to confirm\n' + request.build_absolute_uri(reverse('confirmation', args=[token]))
    subject = 'Confirmation to start a seller'
    message = confirmation_url
    from_email = 'dttung2113@gmail.com'
